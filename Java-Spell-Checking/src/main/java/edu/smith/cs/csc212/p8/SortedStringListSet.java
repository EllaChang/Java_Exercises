...
public class SortedStringListSet extends AbstractSet<String> {
	...
	private int binarySearch(String query, int start, int end) {
		...
		if (end-1 >= start) {
            ...
            // If the element is found in the middle, return the index.
            if (data.get(mid).equals(query))
               return mid;
            
            // If element is smaller than mid, recurse the left half.
            else if (this.data.get(mid).compareTo(query) > 0)
               return binarySearch(query, start, mid);
  
            // If element is greater than mid, recurse the right half.
            else
